...
// Reflect the current connection state in the nav status pill
function updateNetStatus() {
    const online = navigator.onLine;
    document.querySelectorAll('.net-status-display').forEach(el => {
...
async function syncOfflineReflections() {
    ...
    if (syncedCount > 0) {
        showNotification(`Synced ${syncedCount} reflection(s).`);
        ...
        <div class="nav-mobile-right">
            <div class="nav-info-wrapper">
                 <div class="nav-info">
                    ...
                    <div class="net-status net-status-display"><span class="net-dot"></span><span class="net-label">Online</span></div>
                </div>
            </div>
...
function initializeFeatures() {
    ...
    // Show the current connection state in the nav
    updateNetStatus();
